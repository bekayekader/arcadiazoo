const initMenu = (groupe = "_", page = "_") => {
    ...
    try {
      menu.map((smenu, cley) =>{
        let open = ""; if(groupe == smenu.nom) open = " has-active"
        if(smenu.type == "lien" ){
          if(smenu.smenu.length) {
           
          
          let open = "", isopen; if(groupe == smenu.nom){
            open = " show"; isopen = true;
          } 
         ...
         <li>
                        ...
                        <ul class="nav-second-level" aria-expanded="${isopen}">
                            ...
                            smenu.smenu.map(lien =>{
                              let activ = ""; if(page == lien.nom) activ = " active"
                              mn = `${mn}
                              <li class="nav-item" title="${lien.aide || lien.nom }"><a class="nav-link ${activ}" href="${lien.lien}"><i class="ti-control-record"></i>${lien.nom}</a></li>`
                            })	
					mn = `${mn}
          </ul>
				</li>
